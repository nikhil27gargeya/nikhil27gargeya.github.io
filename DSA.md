...
def max_value(nums):
    ...
    # Set max to the first number, then we will compare with the rest of the numbers
    max = nums[0]

    # 3, 4, 5, 6
    # n = 4, iteration happens for index 1 to index 3
    for i in range(1, n):
        if nums[i] > max:
            max = nums[i]
...
def longest_word(sentence):
    ...
    # Track the length of the current word and store it as max.
    # Anytime we find a longer word, we can update the variable.
    arr = sentence.split(" ")
    n = len(arr)
...
Big O: notation to describe performance of algorithms and how performance scales with input size
O(1) constant < O(log n) logarithmic < O(n) linear < O(n^c) polynomial < O(c^n) exponential < O(n!) factorial
and the way to compute what's larger and smaller is just to use real numbers

Time Complexity: how the steps of the algorithm scale with input size Space Complexity: how the space need for the algorithm scales with the input size O(1) < O(logn) < O(n) < O(n^c) < O(c^n) < O(n!)
